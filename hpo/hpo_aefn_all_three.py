...
from models.efn import aefn_all_three
...
def suggest_config(trial, epochs, patience):
    ...
    config["attention_dropout"] = trial.suggest_categorical(
        "attention_dropout", [0.0, 0.1, 0.2]
    )

    config["global_tokens"] = trial.suggest_categorical(
        "global_tokens", [2, 4, 8]
    )
...
def main():
    ...
    # AEFN uses only the particle tensor.
    X_model = X_parts
    ...
    def objective(trial):
        ...
        for fold_info in folds:
            ...
            (
                ...
                _,
                extra_info,
            ) = aefn_all_three.prepare_fold_inputs(
                X_model,
                train_idx,
                val_idx,
                val_idx,
            ...
            model = aefn_all_three.build_model(
                config,
                extra_info,
            )
        ...
        final_auc = float(np.mean(fold_aucs))

        trial.set_user_attr("fold_aucs", fold_aucs)
        trial.set_user_attr(
            "auc_std",
            float(np.std(fold_aucs)),
        )
    ...
    study = optuna.create_study(
        study_name="pbpb_aefn_all_three_reduced_hpo",
        direction="maximize",
        storage=storage,
        load_if_exists=True,
    ...
    result = {
        "model": "aefn_all_three",
        "best_value": study.best_value,
        "best_params": study.best_params,
        "best_trial_number": study.best_trial.number,
        "best_trial_fold_aucs":
            study.best_trial.user_attrs.get("fold_aucs"),
        "best_trial_auc_std":
            study.best_trial.user_attrs.get("auc_std"),
