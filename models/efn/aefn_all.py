...
def get_default_config() -> dict:
    return {
        "model_name": "aefn_all",
        "results_dir_name": "aefn_all_results",
...
def build_model(config: dict, extra_info: dict | None = None):
    ...
    Structure:
        input_p -> Attention -> Phi -> Attention -> z-weighted sum -> Attention -> F -> output
    """
    ...
    x = TimeDistributed(
        Dense(attention_dim, activation=activation),
        name="pre_phi_attention_projection",
    )(x)

    x = Dropout(
        phi_dropout,
        name="pre_phi_attention_projection_dropout",
    )(x)
    ...
    x = apply_attention_blocks(
        ...
        attention_dropout=attention_dropout,
        activation=activation,
        block_prefix="pre_phi_attention",
    )
    ...
    # ------------------------------------------------------------------
    # Important:
    # After the z-weighted sum, the particle dimension is already gone.
    # This is event-level attention, not particle-level attention.
    #
    # We add a sequence dimension of length 1:
    # latent:     (batch, attention_dim)
    # latent_seq: (batch, 1, attention_dim)
    # ------------------------------------------------------------------
    latent = Dense(
        attention_dim,
        activation=activation,
        name="event_attention_projection",
    )(latent)

    latent_seq = Lambda(
        lambda t: tf.expand_dims(t, axis=1),
        name="event_attention_expand",
    )(latent)

    event_attention_mask = Lambda(
        lambda t: tf.ones(
            (tf.shape(t)[0], 1, 1),
            dtype=tf.bool,
        ),
        name="event_attention_mask",
    )(latent_seq)

    latent_seq = apply_attention_blocks(
        x=latent_seq,
        ...
        attention_dropout=attention_dropout,
        activation=activation,
        block_prefix="event_attention",
    )

    latent = Lambda(
        lambda t: tf.squeeze(t, axis=1),
        name="event_attention_squeeze",
    )(latent_seq)
